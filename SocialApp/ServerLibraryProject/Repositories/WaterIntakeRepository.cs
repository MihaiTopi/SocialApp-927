namespace ServerLibraryProject.Repositories
{
    ...
    public class WaterIntakeRepository : IWaterIntakeRepository
    {
        ...
        {
            var existing = this._dbContext.WaterTrackers.FirstOrDefault(w => w.U_Id == userId);
            if (existing == null)
            {
                var user = this._dbContext.Users.FirstOrDefault(u => u.Id == userId);
                if (user == null)
                {
                ...
                this._dbContext.WaterTrackers.Add(newWater);
        ...
        {
            var tracker = this._dbContext.WaterTrackers.FirstOrDefault(w => w.U_Id == userId);
        ...
        public void RemoveWater300(long userId) => this.RemoveWater(userId, 0.3f);

        public void RemoveWater400(long userId) => this.RemoveWater(userId, 0.4f);

        public void RemoveWater500(long userId) => this.RemoveWater(userId, 0.5f);

        public void RemoveWater750(long userId) => this.RemoveWater(userId, 0.75f);
